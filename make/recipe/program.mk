...
_MTB_RECIPE__OPENOCD_CUSTOM_COMMAND?=$(_MTB_RECIPE__OPENOCD_DRIVER_NAME) allow_efuse_program off;

_MTB_RECIPE__OPENOCD_ERASE=init; reset init; $(_MTB_RECIPE__OPENOCD_DRIVER_NAME) sflash_restrictions 1; erase_all; exit;
_MTB_RECIPE__OPENOCD_PROGRAM=$(_MTB_RECIPE__OPENOCD_DRIVER_NAME) sflash_restrictions 1; program $(_MTB_RECIPE__OPENOCD_PROGRAM_IMG) verify reset exit;
...
_MTB_RECIPE__OPENOCD_ERASE_ARGS=$(_MTB_RECIPE__OPENOCD_SCRIPTS) -c \
					"$(_MTB_RECIPE__OPENOCD_INTERFACE) $(_MTB_RECIPE__OPENOCD_CM71_DISABLE_FLAG); $(_MTB_RECIPE__OPENOCD_SMIF_DISABLE); $(_MTB_RECIPE__OPENOCD_TARGET) $(_MTB_RECIPE__OPENOCD_CUSTOM_COMMAND) $(_MTB_RECIPE__OPENOCD_ERASE)"
_MTB_RECIPE__OPENOCD_PROGRAM_ARGS=$(_MTB_RECIPE__OPENOCD_SCRIPTS) $(_MTB_RECIPE__OPENOCD_QSPI) -c \
					"$(_MTB_RECIPE__OPENOCD_INTERFACE) $(_MTB_RECIPE__OPENOCD_CM71_DISABLE_FLAG); $(_MTB_RECIPE__OPENOCD_TARGET) $(_MTB_RECIPE__OPENOCD_CUSTOM_COMMAND) $(_MTB_RECIPE__OPENOCD_PROGRAM)"
_MTB_RECIPE__OPENOCD_DEBUG_ARGS=$(_MTB_RECIPE__OPENOCD_SCRIPTS) $(_MTB_RECIPE__OPENOCD_QSPI) -c \
					"$(_MTB_RECIPE__OPENOCD_INTERFACE) $(_MTB_RECIPE__OPENOCD_CM71_DISABLE_FLAG); $(_MTB_RECIPE__OPENOCD_TARGET) $(_MTB_RECIPE__OPENOCD_CUSTOM_COMMAND) $(_MTB_RECIPE__OPENOCD_DEBUG)"

_MTB_RECIPE__JLINK_DEBUG_ARGS=-if swd -device $(_MTB_RECIPE__JLINK_DEVICE_CFG_DEBUG) -endian little -speed auto -port 2337 -swoport 2338 -telnetport 2339 -vd -ir -localhostonly 1 -singlerun -strict -timeout 0 -nogui